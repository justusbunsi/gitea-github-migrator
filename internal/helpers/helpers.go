...
// Since GitHub has a description size limit for everything, this is something generic to apply.
func SmartRenovateBodyTruncate(str string) string {
	r := regexp.MustCompile(`(?ms)(?P<preNotes>.*### Release Notes)(?P<releaseNotes>.*\n\n</details>\n\n---\n\n)(?P<postNotes>.*)`)
	str = strings.ReplaceAll(str, "was migrated from Gitea", "was migrated from Gitea **and the description was truncated due to platform limits**")
